...
const DIGITAL_TWIN = {
  ...
  financials: { fy2025_revenue: "$536M bookings / ~$509M revenue [8K-Q4-25]", fy2026_guidance_raised: "$516–526M revenue, $114–120M Adj. EBITDA [EC-Q3-26]", q3_fy2026_revenue: "$147.3M (+9.6% YoY, ALL ORGANIC) [EC-Q3-26]", q3_fy2026_gross_margin: "46.6% (+40bps YoY) [EC-Q3-26]", q3_fy2026_adj_ebitda: "$35.6M (24.2% margin) [EC-Q3-26]", q3_fy2026_backlog: "$259.4M (+10.1% YoY) [EC-Q3-26]", net_debt_dec2025: "$96.3M, leverage 0.8x [10-Q3-26]", credit_facility: "$125M term loan + $115M revolver, due Jul 24 2030, JPMorgan Chase [CRED-25]", borrowers: "Thermon Holding Corp (US), Thermon Canada Inc, Thermon Europe B.V. [CRED-25]", market_cap: "~$1.5B (Feb 5 2026) [EC-Q3-26]", revenue_mix: ">70% from non-oil-and-gas end markets [10-K25]" },
...
const H = ({ c }) => <div style={{ fontFamily: M, fontSize: "8px", color: "#3a6", letterSpacing: "0.16em", marginBottom: "6px", paddingBottom: "3px", borderBottom: "1px solid #1a2a1a" }}>▸ {c}</div>;
const R = ({ l, v, hi }) => <div style={{ display: "flex", gap: "8px", padding: "4px 0", borderBottom: "1px solid #0f1520" }}><div style={{ width: "170px", flexShrink: 0, fontFamily: M, fontSize: "8px", color: "#445", lineHeight: "1.4" }}>{l}</div><div style={{ flex: 1, fontFamily: M, fontSize: hi ? "10px" : "8px", color: hi ? "#e87070" : "#aab", lineHeight: "1.5" }}>{v}</div></div>;
...
export default function App() {
  ...
  return (
    <div style={{ background: "#070c12", minHeight: "100vh", color: "#ccd", fontFamily: S, fontSize: "12px" }}>
      ...
      <div style={{ background: "#0b1018", borderBottom: "1px solid #1a2530", padding: "7px 14px", display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: "6px" }}>
        <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
          <span style={{ fontFamily: M, fontSize: "9px", color: "#3a6", letterSpacing: "0.15em" }}>WORLD MODEL v0.3</span>
          <span style={{ fontFamily: M, fontSize: "7px", color: "#334" }}>THERMON GROUP HOLDINGS DIGITAL TWIN · Mar 11 2026</span>
        </div>
        <div style={{ display: "flex", gap: "8px", fontFamily: M, fontSize: "7px", flexWrap: "wrap" }}>
      ...
      <div style={{ background: "#100a04", borderBottom: "1px solid #3a2010", padding: "4px 14px", fontFamily: M, fontSize: "7px", display: "flex", gap: "10px", flexWrap: "wrap" }}>
      ...
      <div style={{ background: "#0b1018", borderBottom: "1px solid #1a2530", padding: "0 14px", display: "flex", flexWrap: "wrap" }}>
        {[["overview","OVERVIEW"],["world","WORLD MODEL"],["twin","DIGITAL TWIN"],["exposure","EXPOSURES"],["growth","GROWTH"],["comp","COMPETITIVE"],["hs","HS MAP"]].map(([id, lbl]) => (
          <button key={id} onClick={() => setTab(id)} style={{ background: "none", border: "none", borderBottom: tab===id?"2px solid #3a6":"2px solid transparent", color: tab===id?"#3a6":"#334", fontFamily: M, fontSize: "8px", letterSpacing: "0.12em", padding: "7px 9px" }}>{lbl}</button>
        ))}
      </div>
      ...
      <div style={{ padding: "12px 14px", maxWidth: "1400px" }}>
        ...
        {tab === "overview" && (
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "9px" }}>
            <div style={{ background: "#0d1520", border: "1px solid #1a2530", padding: "11px" }}>
              ...
              <div style={{ fontFamily: M, fontSize: "14px", color: "#dde", marginBottom: "2px" }}>{tw.firm}</div>
              <div style={{ fontFamily: M, fontSize: "7px", color: "#334", marginBottom: "7px" }}>{tw.hq}</div>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "4px", marginBottom: "8px" }}>
                {[["FY2025 Revenue","~$509M [8K-Q4-25]"],["FY2026 Guidance","$516–526M raised [EC-Q3-26]"],["Q3 FY2026 Revenue","$147.3M (+10% organic) [EC-Q3-26]"],["Q3 Adj. EBITDA","$35.6M · 24.2% margin"],["Backlog Dec 2025","$259.4M (+10%) [EC-Q3-26]"],["Net Leverage","0.8x [10-Q3-26]"],["Employees","1,568 + 227 contingent [10-K25]"],["Market Cap","~$1.5B [EC-Q3-26]"]].map(([k,v]) => (
                  <div key={k} style={{ padding: "4px 6px", background: "#080c12", borderLeft: "2px solid #1a3020" }}>
                    <div style={{ fontFamily: M, fontSize: "6px", color: "#334", marginBottom: "1px" }}>{k}</div>
                    <div style={{ fontFamily: M, fontSize: "8px", color: "#aab" }}>{v}</div>
                  </div>
                ))}
              </div>
              <H c="ACQUISITIONS" />
              {tw.acquisitions.map(a => (
                <div key={a.name} style={{ padding: "5px 6px", background: "#080c12", borderLeft: "2px solid #1a2530", marginBottom: "3px" }}>
                  <span style={{ fontFamily: M, fontSize: "8px", color: "#3a6" }}>{a.name}</span>
                  <span style={{ fontFamily: M, fontSize: "7px", color: "#334", marginLeft: "7px" }}>{a.date} · {a.price}</span>
                  <div style={{ fontFamily: S, fontSize: "10px", color: "#556", marginTop: "2px" }}>{a.description.slice(0,85)}…</div>
                </div>
              ))}
            </div>
            <div style={{ background: "#0d1520", border: "1px solid #1a2530", padding: "11px" }}>
              ...
              {Object.entries(WORLD_MODEL).map(([key, w]) => (
                ...
                >
                  <div style={{ fontFamily: M, fontSize: "9px", color: "#ccd", width: "58px", flexShrink: 0 }}>{key}</div>
                  <div style={{ flex: 1, fontFamily: M, fontSize: "7px", color: "#334", lineHeight: "1.4" }}>{(w.tariff.applied_non_usmca || w.tariff.applied || w.tariff.applied_stack || "").slice(0,48)}</div>
                  <Badge v={w.tariff.volatility} sm />
                  <div style={{ fontFamily: M, fontSize: "7px", color: "#2a5", whiteSpace: "nowrap" }}>{w.tariff.status.split("·")[0].trim()}</div>
                </div>
              ))}
            </div>
                ...
                {tw.disclosed_exposures.filter(e => e.severity==="HIGH").map((e, i) => (
                  <div key={i} style={{ background: "#100808", border: "1px solid #2d1010", padding: "8px" }}>
                    <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "4px" }}>
                      <div style={{ fontFamily: M, fontSize: "7px", color: "#e87070", flex: 1, paddingRight: "5px", lineHeight: "1.4" }}>{e.category.toUpperCase()}</div>
                      <Badge v={e.severity} sm />
                    </div>
                    <div style={{ fontFamily: M, fontSize: "8px", color: "#3a6", marginBottom: "2px" }}>NOW: {e.current_rate.split("[")[0].trim()}</div>
                    {e.prior_rate && e.prior_rate!=="N/A" && <div style={{ fontFamily: M, fontSize: "7px", color: "#334", marginBottom: "3px" }}>PRIOR: {e.prior_rate.slice(0,45)}…</div>}
                    <div style={{ fontFamily: S, fontSize: "10px", color: "#778", lineHeight: "1.5" }}>{e.detail.slice(0,95)}…</div>
                  </div>
                ))}
              </div>
            </div>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: "5px" }}>
                {Object.entries(tw.tariff_quantification).map(([k,v]) => (
                  <div key={k} style={{ padding: "6px 8px", background: "#0d1520", borderLeft: "2px solid #1a3020" }}>
                    <div style={{ fontFamily: M, fontSize: "7px", color: "#334", marginBottom: "2px" }}>{k.replace(/_/g," ").toUpperCase()}</div>
                    <div style={{ fontFamily: S, fontSize: "10px", color: "#778", lineHeight: "1.5" }}>{v}</div>
                  </div>
                ))}
              </div>
            </div>
        ...
        {tab === "world" && (
          <div style={{ display: "grid", gridTemplateColumns: "130px 1fr", gap: "9px" }}>
            <div style={{ background: "#0d1520", border: "1px solid #1a2530", padding: "9px" }}>
              <div style={{ fontFamily: M, fontSize: "7px", color: "#334", marginBottom: "5px" }}>PAIRS</div>
              {Object.keys(WORLD_MODEL).map(key => (
                <button key={key} onClick={() => setPair(key)} style={{ display: "block", width: "100%", textAlign: "left", background: pair===key?"#0f1e10":"none", border: `1px solid ${pair===key?"#1e3":"transparent"}`, color: pair===key?"#3a6":"#334", fontFamily: M, fontSize: "9px", padding: "5px 6px", marginBottom: "2px" }}>{key}</button>
              ))}
            </div>
            <div style={{ background: "#0d1520", border: "1px solid #1a2530", padding: "13px" }}>
              <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "10px" }}>
                <div>
                  <div style={{ fontFamily: M, fontSize: "7px", color: "#334", marginBottom: "2px" }}>BILATERAL PAIR · {wm.data_cutoff}</div>
                  <div style={{ fontFamily: M, fontSize: "15px", color: "#dde" }}>{wm.label}</div>
                </div>
                <div style={{ textAlign: "right" }}>
                  <Badge v={wm.tariff.volatility} />
                  <div style={{ fontFamily: M, fontSize: "8px", color: "#2a5", marginTop: "3px" }}>{wm.tariff.status}</div>
                </div>
              </div>
              ...
              {wm.hs_codes && (
                <div style={{ marginBottom: "12px" }}>
                  ...
                  <div style={{ display: "grid", gridTemplateColumns: "80px 150px 1fr 1fr 52px", gap: "3px", padding: "3px 6px", background: "#080c12", fontFamily: M, fontSize: "7px", color: "#334", marginBottom: "2px" }}>
                  ...
                  {wm.hs_codes.map((h, i) => (
                    <div key={i} style={{ display: "grid", gridTemplateColumns: "80px 150px 1fr 1fr 52px", gap: "3px", padding: "5px 6px", background: i%2===0?"#080c12":"transparent", borderBottom: "1px solid #0f1520" }}>
                      <div style={{ fontFamily: M, fontSize: "8px", color: "#3a6" }}>{h.code}</div>
                      <div style={{ fontFamily: S, fontSize: "9px", color: "#778", lineHeight: "1.4" }}>{h.desc}</div>
                      <div style={{ fontFamily: M, fontSize: "8px", color: "#aab" }}>{h.mfn}</div>
                      <div style={{ fontFamily: M, fontSize: "8px", color: h.total?"#e87070":"#d4942a" }}>{h.total||h.non_usmca||h.applied||h.applied_non_usmca}</div>
                      <div style={{ fontFamily: M, fontSize: "8px", color: "#334" }}>{h.s232||"—"}</div>
                    </div>
                  ))}
                </div>
              )}
                  ...
                  {wm.ntb.map((n, i) => (
                    <div key={i} style={{ padding: "7px", background: "#080c12", border: "1px solid #1a2530", marginBottom: "3px" }}>
                      <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "3px" }}>
                        <div style={{ fontFamily: M, fontSize: "8px", color: "#aab" }}>{n.type}</div>
                        <Badge v={n.severity} sm />
                      </div>
                      <div style={{ fontFamily: S, fontSize: "10px", color: "#667", lineHeight: "1.5" }}>{n.detail}</div>
                    </div>
                  ))}
                </div>
              )}
              <div style={{ marginTop: "7px", paddingTop: "5px", borderTop: "1px solid #0f1520", fontFamily: M, fontSize: "7px", color: "#334" }}>SOURCES: {wm.sources.join(" · ")}</div>
            </div>
          </div>
        )}
        ...
        {tab === "twin" && (
          <div style={{ display: "grid", gridTemplateColumns: "150px 1fr", gap: "9px" }}>
            <div style={{ background: "#0d1520", border: "1px solid #1a2530", padding: "9px" }}>
              <div style={{ fontFamily: M, fontSize: "7px", color: "#334", marginBottom: "5px" }}>SEGMENTS</div>
              <button onClick={() => setSeg(null)} style={{ display: "block", width: "100%", textAlign: "left", background: !seg?"#0f1e10":"none", border: `1px solid ${!seg?"#1e3":"transparent"}`, color: !seg?"#3a6":"#334", fontFamily: M, fontSize: "8px", padding: "4px 6px", marginBottom: "2px" }}>ALL</button>
              {tw.segments.map(s => (
                <button key={s.name} onClick={() => setSeg(s.name)} style={{ display: "block", width: "100%", textAlign: "left", background: seg===s.name?"#0f1e10":"none", border: `1px solid ${seg===s.name?"#1e3":"transparent"}`, color: seg===s.name?"#3a6":"#334", fontFamily: M, fontSize: "7px", padding: "4px 6px", marginBottom: "2px", lineHeight: "1.5" }}>
              ...
              {(seg ? tw.segments.filter(s => s.name===seg) : tw.segments).map(s => (
                <div key={s.name} style={{ background: "#0d1520", border: "1px solid #1a2530", padding: "11px", marginBottom: "7px" }}>
                  <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "7px" }}>
                    <div>
                      <div style={{ fontFamily: M, fontSize: "12px", color: "#dde" }}>{s.name}</div>
                      <div style={{ fontFamily: M, fontSize: "8px", color: "#3a6" }}>{s.revenue_share} · {s.revenue_fy2025} FY2025 · Q3 FY2026: {s.q3_fy2026}</div>
                    </div>
                    <div style={{ display: "flex", gap: "3px", flexWrap: "wrap", justifyContent: "flex-end" }}>
                      {s.relevant_pairs?.map(p => <button key={p} onClick={() => { setPair(p); setTab("world"); }} style={{ fontFamily: M, fontSize: "7px", background: "#0f1e10", border: "1px solid #1e3", color: "#3a6", padding: "2px 5px" }}>{p} ↗</button>)}
                    </div>
                  </div>
                  <H c="MANUFACTURING FOOTPRINT" />
                  <div style={{ display: "flex", flexWrap: "wrap", gap: "3px", marginBottom: "7px" }}>
                    {s.manufacturing.map((m, i) => <div key={i} style={{ fontFamily: M, fontSize: "7px", color: "#667", background: "#080c12", border: "1px solid #1a2530", padding: "2px 6px" }}>{m}</div>)}
                  </div>
                  <div style={{ fontFamily: S, fontSize: "11px", color: "#667", lineHeight: "1.6", borderLeft: "2px solid #1a3020", paddingLeft: "9px" }}>{s.notes}</div>
                  {s.fx && <div style={{ fontFamily: M, fontSize: "8px", color: "#334", marginTop: "5px", paddingTop: "4px", borderTop: "1px solid #0f1520" }}>{s.fx}</div>}
                </div>
              ))}
              {!seg && (
                <div style={{ background: "#0d1520", border: "1px solid #1a2530", padding: "11px" }}>
                  <H c="SUPPLY CHAIN INPUTS — HTS + TARIFF MAPPING" />
                  <div style={{ display: "grid", gridTemplateColumns: "140px 100px 70px 52px 1fr", gap: "4px", padding: "3px 6px", background: "#080c12", fontFamily: M, fontSize: "7px", color: "#334", marginBottom: "2px" }}>
                  ...
                  {tw.supply_chain.map((sc, i) => (
                    <div key={i} style={{ display: "grid", gridTemplateColumns: "140px 100px 70px 52px 1fr", gap: "4px", padding: "5px 6px", background: i%2===0?"#080c12":"transparent", borderBottom: "1px solid #0f1520", alignItems: "start" }}>
                      <div style={{ fontFamily: M, fontSize: "8px", color: "#aab", lineHeight: "1.4" }}>{sc.input}</div>
                      <div style={{ fontFamily: S, fontSize: "9px", color: "#556" }}>{sc.origin}</div>
                      <div style={{ fontFamily: M, fontSize: "7px", color: "#2a5" }}>{sc.tariff_pair||"—"}</div>
                      <Badge v={sc.risk} sm />
                      <div style={{ fontFamily: S, fontSize: "9px", color: "#445", lineHeight: "1.4" }}>{sc.note}</div>
                    </div>
                  ))}
                </div>
              )}
        ...
        {tab === "exposure" && (
          <div style={{ display: "flex", flexDirection: "column", gap: "9px" }}>
            <div style={{ background: "#0d1520", border: "1px solid #1a2530", padding: "11px" }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "140px 52px 115px 115px 1fr 125px", gap: "4px", padding: "3px 6px", background: "#080c12", fontFamily: M, fontSize: "7px", color: "#334", marginBottom: "2px" }}>
              ...
              {tw.disclosed_exposures.map((e, i) => (
                <div key={i} style={{ display: "grid", gridTemplateColumns: "140px 52px 115px 115px 1fr 125px", gap: "4px", padding: "6px 6px", background: i%2===0?"#080c12":"transparent", borderBottom: "1px solid #0f1520", alignItems: "start" }}>
                  <div style={{ fontFamily: M, fontSize: "7px", color: "#aab", lineHeight: "1.4" }}>{e.category}</div>
                  <Badge v={e.severity} sm />
                  <div style={{ fontFamily: M, fontSize: "8px", color: "#e87070", lineHeight: "1.4" }}>{e.current_rate.split("[")[0].trim()}</div>
                  <div style={{ fontFamily: M, fontSize: "7px", color: "#334", lineHeight: "1.4" }}>{e.prior_rate!=="N/A" ? e.prior_rate.slice(0,42) : "—"}</div>
                  <div style={{ fontFamily: S, fontSize: "10px", color: "#667", lineHeight: "1.5" }}>{e.detail}</div>
                  <div style={{ fontFamily: M, fontSize: "7px", color: "#3a6", lineHeight: "1.5" }}>{e.time_horizon}</div>
                </div>
              ))}
            </div>
            <div style={{ background: "#0d1520", border: "1px solid #1a2530", padding: "11px" }}>
              ...
              <div style={{ overflowX: "auto" }}>
                <table style={{ width: "100%", borderCollapse: "collapse", fontFamily: M, fontSize: "8px" }}>
                  <thead><tr>
                    ...
                    {tw.segments.map((s, i) => (
                      <tr key={s.name} style={{ background: i%2===0?"#080c12":"transparent" }}>
                        <td style={{ padding: "5px 6px", color: "#778", borderBottom: "1px solid #0f1520", whiteSpace: "nowrap" }}>
                          {s.name.split("(")[0].trim()}<br /><span style={{ color: "#2a5", fontSize: "7px" }}>{s.revenue_share}</span>
                        </td>
                        {Object.keys(WORLD_MODEL).map(p => {
                          ...
                          return <td key={p} style={{ padding: "5px", textAlign: "center", borderBottom: "1px solid #0f1520" }}>
                            {rel ? <button onClick={() => { setPair(p); setTab("world"); }} style={{ background: "#2d1010", border: "1px solid #8b2020", color: "#e87070", fontFamily: M, fontSize: "7px", padding: "1px 4px" }}>{short}</button>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "9px" }}>
            {Object.entries(tw.growth_platforms).map(([k,v]) => (
              <div key={k} style={{ background: "#0d1520", border: "1px solid #1a2530", padding: "11px" }}>
                <div style={{ fontFamily: M, fontSize: "8px", color: "#3a6", letterSpacing: "0.1em", marginBottom: "5px" }}>{k.replace(/_/g," ").toUpperCase()}</div>
                <div style={{ fontFamily: S, fontSize: "11px", color: "#778", lineHeight: "1.6" }}>{v}</div>
              </div>
            ))}
            <div style={{ gridColumn: "1/-1", background: "#0d1520", border: "1px solid #1a2530", padding: "11px" }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: "5px" }}>
                {[["Alberta total CapEx 2024","Cdn$30.9B (+2.2%) [AER]"],["Oil sands CapEx 2024","Cdn$13.3B (+6.4%) [AER]"],["4 largest producers 2026","$14B (+5% from 2025) [ARGUS]"],["AER tariff scenario","-11.1% CapEx if tariffs persist [AER]"],["CNRL 2026 target","1.62M boe/d (record) [ARGUS]"],["Cenovus 2026 target","985k boe/d [ARGUS]"],["Oil sands breakeven","~$40/bbl — baseload [ARGUS]"],["Alberta data center demand",">20GW proposed — overlaps Poseidon platform"]].map(([k,v]) => (
                  <div key={k} style={{ padding: "5px 6px", background: "#080c12", borderLeft: "2px solid #1a3020" }}>
                    <div style={{ fontFamily: M, fontSize: "7px", color: "#334", marginBottom: "2px" }}>{k}</div>
                    <div style={{ fontFamily: M, fontSize: "8px", color: "#aab" }}>{v}</div>
                  </div>
                ))}
              </div>
            ...
            <div style={{ background: "#0d1520", border: "1px solid #1a2530", padding: "11px" }}>
              ...
              {tw.competitive_landscape.competitors.map((c, i) => (
                <div key={i} style={{ display: "flex", gap: "9px", padding: "5px 6px", background: i%2===0?"#080c12":"transparent", borderBottom: "1px solid #0f1520" }}>
                  <div style={{ fontFamily: M, fontSize: "8px", color: "#3a6", width: "240px", flexShrink: 0 }}>{c.name}</div>
                  <div style={{ fontFamily: S, fontSize: "10px", color: "#667" }}>{c.note}</div>
                </div>
              ))}
            </div>
            <div style={{ background: "#0d1520", border: "1px solid #1a2530", padding: "11px" }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "7px" }}>
                <div style={{ padding: "7px", background: "#080c12", borderLeft: "2px solid #1a3020" }}>
                  <div style={{ fontFamily: M, fontSize: "7px", color: "#3a6", marginBottom: "3px" }}>IN-COUNTRY MANUFACTURING</div>
                  <div style={{ fontFamily: S, fontSize: "10px", color: "#778", lineHeight: "1.6" }}>CEO Bruce Thames Q3 FY2025: "Our manufacturing strategy of producing in-country helps insulate us from tariffs." Five Canadian facilities for Canadian customers; San Marcos TX for US; FATI Milan for Europe. No manufacturing in China or Mexico. [EC-Q3-25]</div>
                </div>
                <div style={{ padding: "7px", background: "#080c12", borderLeft: "2px solid #1a3020" }}>
                  <div style={{ fontFamily: M, fontSize: "7px", color: "#3a6", marginBottom: "3px" }}>GENESIS NETWORK PLATFORM</div>
                  <div style={{ fontFamily: S, fontSize: "10px", color: "#778", lineHeight: "1.6" }}>Proprietary IIoT heat tracing monitoring. Real-time analytics, predictive maintenance, centralized monitoring. Next-generation launched Jan 2024. Creates installed-base stickiness and recurring OpEx revenue stream. [10-K25][M&M]</div>
                </div>
              </div>
            </div>
          </div>
        ...
        {tab === "hs" && (
          <div style={{ background: "#0d1520", border: "1px solid #1a2530", padding: "11px" }}>
            ...
            <div style={{ fontFamily: S, fontSize: "10px", color: "#556", marginBottom: "9px", lineHeight: "1.6" }}>MFN rates: USITC HTS 2025 Rev 29. S301: USTR register Lists 1–4A. S122: 10% global surcharge, Feb 24 2026, expires Jul 24 2026. S232: 25% on steel/aluminum, permanent, unaffected by SCOTUS. USMCA: qualifying goods exempt from S122.</div>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(2, 1fr)", gap: "7px" }}>
              {[
                ...
                <div key={idx} style={{ background: "#080c12", border: "1px solid #1a2530", padding: "9px" }}>
                  <div style={{ fontFamily: M, fontSize: "11px", color: "#3a6", marginBottom: "1px" }}>{item.code}</div>
                  <div style={{ fontFamily: M, fontSize: "8px", color: "#aab", marginBottom: "2px" }}>{item.label}</div>
                  <div style={{ fontFamily: S, fontSize: "9px", color: "#445", marginBottom: "4px", fontStyle: "italic" }}>Thermon: {item.thermon}</div>
                  <div style={{ fontFamily: M, fontSize: "7px", color: "#2a5", marginBottom: "5px" }}>MFN: {item.mfn}</div>
                  {item.rates.map((r, i) => (
                    <div key={i} style={{ display: "flex", gap: "6px", padding: "3px 0", borderBottom: "1px solid #0f1520", alignItems: "start" }}>
                      <div style={{ fontFamily: M, fontSize: "7px", color: "#334", width: "145px", flexShrink: 0 }}>{r.pair}</div>
                      <div style={{ fontFamily: M, fontSize: "8px", color: "#e87070", flex: 1 }}>{r.rate}</div>
                      <div style={{ fontFamily: M, fontSize: "7px", color: "#2a5", whiteSpace: "nowrap" }}>[{r.auth}]</div>
                    </div>
                  ))}
                </div>
              ))}
            </div>
            <div style={{ marginTop: "9px", paddingTop: "6px", borderTop: "1px solid #0f1520", fontFamily: M, fontSize: "7px", color: "#2a3" }}>SOURCES: USITC HTS 2025 Rev 29 · USTR S301 Register · S122 Proclamation Feb 20 2026 · S232 Proclamation · SCOTUS Feb 20 2026 · BDO Mar 2026 · Fasken Feb 2026 · Federal Circuit Sep 2025 · Thermon EC-Q3-25 / EC-Q4-25 / EC-Q3-26 · 10-K FY2025</div>
          </div>
        )}
